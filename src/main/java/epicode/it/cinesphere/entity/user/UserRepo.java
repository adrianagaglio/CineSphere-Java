...
public interface UserRepo extends JpaRepository<User, Long> {
    ...
    @Query("SELECT u.id as id, u.firstName as firstName, u.lastName as lastName, au.email as email, au.username as username FROM User u JOIN AppUser au ON au.id = u.appUser.id WHERE au.username = :username")
    public IGetUserResponse findFirstByUsernameIgnoreCase(String username);

    @Query("SELECT u.id as id,  u.firstName as firstName, u.lastName as lastName, au.email as email, au.username as username FROM User u JOIN AppUser au ON au.id = u.appUser.id WHERE au.email = :email")
    ...
    @Query("SELECT u.id as id, u.firstName as firstName, u.lastName as lastName, au.username as username, au.email as email, au.roles FROM User u JOIN AppUser au ON au.id = u.appUser.id")
    public List<IGetUserResponse> findGetUsersResponseBy();

    @Query("SELECT u.id as id, u.firstName as firstName, u.lastName as lastName, au.username as username, au.email as email,au.roles FROM User u JOIN AppUser au ON u.id = au.id WHERE u.id = :id")
