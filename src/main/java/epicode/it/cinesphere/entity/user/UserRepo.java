...
public interface UserRepo extends JpaRepository<User, Long> {
    ...
    @Query("SELECT u.firstName, u.lastName, au.username, au.email, au.roles FROM User u JOIN AppUser au ON au.id = u.appUser.id")
    public List<IGetUserResponse> findGetUsersResponseBy();

    @Query("SELECT u.firstName, u.lastName, au.username, au.email,au.roles FROM User u JOIN AppUser au ON u.id = au.id WHERE u.id = :id")
